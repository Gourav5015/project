...
class home(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        image = ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED") .filterDate('2021-10-01', '2021-10-31').min()
...
class airQualityIndexView(generics.GenericAPIView):
    def get(self, request):
        ...
        data_dict = {
            'name': 'Air Quality Analysis of Kolkata',
            'description': 'Map representing air quality of Kolkata',
...
class landCoverView(generics.GenericAPIView):
    def get(self, request):
        ...
        data_dict = {
            'name': 'Landcover Analysis of Kolkata',
            'description': 'Map representing predicted landcover of Kolkata using supervised classification technique',
...
class ndviTimelapseView(generics.GenericAPIView):
    def get(self, request):
        dataset_list=["UHarmonized Sentinel-2 MSI: MultiSpectral Instrument, Level-2A ",] 

        data_dict = {
            'name': 'Timelapse for Normalized Difference Vegetation Index (NDVI)',
            'description': 'Map to generate timelapse for Normalized Difference Vegetation Index (NDVI) of selected region',
...
class vegetationView(generics.GenericAPIView):
    def get(self, request):
        dataset_list=["Harmonized Sentinel-2 MSI: MultiSpectral Instrument, Level-2A "]

        data_dict = {
            'name': 'Estimating Green Cover using Vegetation Indexes',
    ...
    def post(self, request):
        ...
        if int(st_date) < 20170218 or int(ed_date) < 20170218:
        ...
        Maharashtra = admin2.filter(ee.Filter.eq('ADM1_NAME', 'West Bengal'))
        ...
        print(m_ndvi.dataframe.index, m_ndvi.dataframe['NDVI'])
        ax.bar(m_ndvi.dataframe.index, m_ndvi.dataframe['NDVI'],
                color='forest green')
        ax.set_ylabel('NDVI')
